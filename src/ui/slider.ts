...
export class DxSliderComponent extends DxComponent {
    ...
    protected _createInstance(element, options) {
        let widget = new DxSlider(element, options);
        if (this.validator) {
...
const CUSTOM_VALUE_ACCESSOR = {
    ...
    useExisting: forwardRef(() => DxSliderValueAccessorDirective),
    multi: true
};
...
export class DxSliderValueAccessorDirective implements ControlValueAccessor {
    @HostListener('valueChange', ['$event']) onChange(_) { }
    onTouched = () => {};

    constructor(private host: DxSliderComponent) { }
