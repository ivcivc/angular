...
export class DxCheckBoxComponent extends DxComponent {
    ...
    protected _createInstance(element, options) {
        let widget = new DxCheckBox(element, options);
        if (this.validator) {
...
const CUSTOM_VALUE_ACCESSOR = {
    ...
    useExisting: forwardRef(() => DxCheckBoxValueAccessorDirective),
    multi: true
};
...
export class DxCheckBoxValueAccessorDirective implements ControlValueAccessor {
    @HostListener('valueChange', ['$event']) onChange(_) { }
    onTouched = () => {};

    constructor(private host: DxCheckBoxComponent) { }
