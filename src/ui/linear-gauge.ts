...
export class DxLinearGaugeComponent extends DxComponent implements OnChanges, DoCheck {
    ...
    protected _createInstance(element, options) {
        let widget = new DxLinearGauge(element, options);
        if (this.validator) {
...
const CUSTOM_VALUE_ACCESSOR = {
    ...
    useExisting: forwardRef(() => DxLinearGaugeValueAccessorDirective),
    multi: true
};
...
export class DxLinearGaugeValueAccessorDirective implements ControlValueAccessor {
    @HostListener('valueChange', ['$event']) onChange(_) { }
    onTouched = () => {};

    constructor(private host: DxLinearGaugeComponent) { }
