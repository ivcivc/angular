...
export class DxAutocompleteComponent extends DxComponent implements OnChanges, DoCheck {
    ...
    @Input()
    get maxItemCount(): any {
        return this._getOption('maxItemCount');
    }

    set maxItemCount(value: any) {
    ...
    @Output() maxItemCountChange: EventEmitter<any>;
    ...
    constructor(elementRef: ElementRef, ngZone: NgZone, templateHost: DxTemplateHost, private _watcherHelper: WatcherHelper,
        ...
        this.maxItemCountChange = new EventEmitter();

        this._idh.setHost(this);
    }

    protected _createInstance(element, options) {
        let widget = new DxAutocomplete(element, options);
        if (this.validator) {
...
const CUSTOM_VALUE_ACCESSOR = {
    ...
    useExisting: forwardRef(() => DxAutocompleteValueAccessorDirective),
    multi: true
};
...
export class DxAutocompleteValueAccessorDirective implements ControlValueAccessor {
    @HostListener('valueChange', ['$event']) onChange(_) { }
    onTouched = () => {};

    constructor(private host: DxAutocompleteComponent) { }
