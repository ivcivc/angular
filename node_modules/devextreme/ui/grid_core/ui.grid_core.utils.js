...
exports.equalKeys = function(key1, key2, level) {
    var propertyName, i;
    level = level || 0;
    if (level < MAX_EQUAL_KEYS_LEVEL) {
        if (commonUtils.isObject(key1) && commonUtils.isObject(key2)) {
            for (propertyName in key1) {
                if (key1.hasOwnProperty(propertyName) && !exports.equalKeys(key1[propertyName], key2[propertyName], level + 1)) {
                    return false
                }
            }
            ...
            if (commonUtils.isArray(key1) && commonUtils.isArray(key2)) {
                if (key1.length !== key2.length) {
                    return false
                }
                for (i = 0; i < key1.length; i++) {
                    if (!exports.equalKeys(key1[i], key2[i], level + 1)) {
                        return false
                    }
                }
                return true
            } else {
                if (commonUtils.isDate(key1) && commonUtils.isDate(key2)) {
