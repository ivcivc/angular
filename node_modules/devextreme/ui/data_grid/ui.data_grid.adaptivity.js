...
var $ = require("jquery"),
    ...
    DATAGRID_ADAPTIVE_COLUMN_CLASS = "dx-command-adaptive",
    DATAGRID_ADAPTIVE_COLUMN_NAME = "adaptive",
    DATAGRID_ADAPTIVE_DETAIL_ROW_CLASS = "dx-adaptive-detail-row",
    DATAGRID_ADAPTIVE_HIDDEN_COLUMN_NOWRAP_CLASS = "dx-hidden-column-nowrap",
...
exports.AdaptiveColumnsController = gridCore.ViewController.inherit({
    ...
    _getFormViewTemplate: function(item, cellOptions) {
        var cellText, that = this,
            column = item.column,
            cellValue = column.calculateCellValue(cellOptions.data),
            templateAdapter = column.cellTemplate ? function(options, $container) {
        ...
        return templateAdapter || function(options, $container) {
    ...
    _getTemplate: function(item, cellOptions) {
        var column = item.column,
            ...
            isReadOnly = !this._isItemEdited(item);
        if (!isReadOnly || column.showEditorAlways) {
            return editingController.getFormEditorTemplate(cellOptions, column, item, isReadOnly)
        } else {
            return this._getFormViewTemplate(item, cellOptions)
        }
    },
    ...
    _isItemEdited: function(item) {
        ...
        if (this._isRowEditMode()) {
            ...
            if (gridCoreUtils.equalKeys(editRowKey, this._dataController.adaptiveExpandedKey())) {
                return true
            }
        } else {
    ...
    _renderAdaptiveDetailRowPreparingAction: function(actionFunc) {
        var action = actionFunc || "onAdaptiveDetailRowPreparing";
    ...
    _restoreWordWrappingForColumn: function() {
        var $cells = this._rowsView.element().find("." + DATAGRID_ADAPTIVE_HIDDEN_COLUMN_NOWRAP_CLASS);
        $cells.removeClass(DATAGRID_ADAPTIVE_HIDDEN_COLUMN_NOWRAP_CLASS)
    },
    _disableWordWrappingForColumn: function(visibleIndex) {
        var $cell, rowIndex, items = this._dataController.items();
        for (rowIndex = 0; rowIndex < items.length; rowIndex++) {
            $cell = this._rowsView.getCellElements(rowIndex).eq(visibleIndex);
            $cell.addClass(DATAGRID_ADAPTIVE_HIDDEN_COLUMN_NOWRAP_CLASS)
        }
    },
    hideRedundantColumns: function(resultWidths, visibleColumns, hiddenQueue) {
        var visibleColumn, that = this;
        this._hiddenColumns = [];
        this._restoreWordWrappingForColumn();
        if (that._isVisibleColumnsValid(visibleColumns) && hiddenQueue.length) {
            ...
            do {
                ...
                if (needHideColumn) {
                    ...
                    that._disableWordWrappingForColumn(visibleIndex);
                    resultWidths[visibleIndex] = DATAGRID_HIDDEN_COLUMNS_WIDTH;
                    contentColumnCount--;
    ...
    init: function() {
        ...
        that._columnsController.addCommandColumn({
            ...
            visible: false,
            cssClass: DATAGRID_ADAPTIVE_COLUMN_CLASS,
            width: "auto"
        });
        ...
        that._hidingColumnsQueue = [];
        that._hiddenColumns = [];
        that._renderAdaptiveDetailRowPreparingAction();
        that.callBase()
    },
    optionChanged: function(args) {
        if ("onAdaptiveDetailRowPreparing" === args.name) {
            this._renderAdaptiveDetailRowPreparingAction(args.value)
        }
    ...
    isAdaptiveDetailRowExpanded: function(key) {
        return this._dataController.adaptiveExpandedKey() && gridCoreUtils.equalKeys(this._dataController.adaptiveExpandedKey(), key)
    },
        ...
        views: {
            rowsView: {
                ...
                isClickableElement: function($target) {
                    ...
                    return isClickable || !!$target.closest("." + DATAGRID_ADAPTIVE_COLUMN_CLASS).length
                },
                init: function() {
                    this.callBase();
        ...
        controllers: {
            ...
            editing: {
                ...
                createHighlightCell: function($cell, skipValidation) {
                    if (!$cell.hasClass(FORM_ITEM_CONTENT_CLASS)) {
                        this.callBase($cell, skipValidation)
                    }
                },
                init: function() {
                    this.callBase();
